...
contract TestContract02_MintableIncentivizedERC20 is Test {
    ...
    function setUp() public {
        ...
        // Deploy token
        token = new MockMintableIncentivizedERC20(yoloHook, address(aclManager), "Mock Token", "MOCK", 18);
    ...
    function test_Contract02_Case11_incentivesAdminCanUpdateTracker() public {
        ...
        emit IncentivesTrackerUpdated(
            IIncentivesTracker(address(incentivesController)), IIncentivesTracker(address(newController))
        );
        token.setIncentivesTracker(newController);
